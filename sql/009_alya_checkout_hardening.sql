...
IF OBJECT_ID('dbo.AlyaOrders','U') IS NOT NULL
BEGIN
  IF COL_LENGTH('dbo.AlyaOrders','CheckoutToken') IS NOT NULL
    AND NOT EXISTS (
      SELECT 1
      FROM sys.indexes
      WHERE name='UX_AlyaOrders_CheckoutToken'
        AND object_id=OBJECT_ID('dbo.AlyaOrders')
    )
    CREATE UNIQUE INDEX UX_AlyaOrders_CheckoutToken
      ON dbo.AlyaOrders(CheckoutToken)
      WHERE CheckoutToken IS NOT NULL;
END;
GO
