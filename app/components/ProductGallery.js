...
function cropProductImage(src) {
  return new Promise(resolve => {
    if (!src || typeof window === 'undefined') return resolve(src);
    ...
    image.onload = () => {
      try {
        ...
        const points = [
          sample(0, 0), sample(sampleWidth - 1, 0),
          sample(0, sampleHeight - 1), sample(sampleWidth - 1, sampleHeight - 1),
          sample(Math.floor(sampleWidth / 2), 0), sample(Math.floor(sampleWidth / 2), sampleHeight - 1),
          sample(0, Math.floor(sampleHeight / 2)), sample(sampleWidth - 1, Math.floor(sampleHeight / 2)),
        ].filter(pixel => pixel[3] > 8);
        if (points.length < 4) return resolve(src);

        const background = [0, 0, 0].map((_, channel) => {
          const values = points.map(pixel => pixel[channel]).sort((a, b) => a - b);
          return values[Math.floor(values.length / 2)];
        });
        const distance = (pixel) => Math.sqrt(
        ...
        if (detectedWidth < sampleWidth * 0.035 || detectedHeight < sampleHeight * 0.035) return resolve(src);
        if (detectedWidth > sampleWidth * 0.985 && detectedHeight > sampleHeight * 0.985) return resolve(src);

        const margin = Math.max(6, Math.round(Math.min(detectedWidth, detectedHeight) * 0.035));
        ...
        if (!outputContext) return resolve(src);
        outputContext.drawImage(image, minX / scale, minY / scale, cropWidth / scale, cropHeight / scale, 0, 0, output.width, output.height);
...
export default function ProductGallery({ product }) {
  ...
  useEffect(() => {
    let active = true;
    setImages([]); setSelectedIndex(0); setDisplayImage(''); setFailed(false); setLightbox(false); setImageOrientation('unknown');
  ...
  useEffect(() => {
    ...
    if (!currentImage) return () => { active = false; };
    cropProductImage(currentImage).then(cropped => {
      if (!active) return;
      setDisplayImage(cropped || currentImage);
    });
    return () => { active = false; };
  }, [currentImage]);
  ...
  const openLightbox = () => { if (displayImage || currentImage) setLightbox(true); };
      ...
      <div className="alya-gallery-layout">
        ...
        <div className="alya-gallery-main">
          ...
          <div className={`alya-product-image is-${imageOrientation}`} onClick={openLightbox} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} role="button" tabIndex={0} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openLightbox(); } }} aria-label="Fotoğrafı tam ekran aç">
            {!failed && (displayImage || currentImage) ? <img src={displayImage || currentImage} alt={`${product.name} - ALYA HOMES`} onLoad={event => { const { naturalWidth, naturalHeight } = event.currentTarget; setImageOrientation(naturalWidth >= naturalHeight ? 'landscape' : 'portrait'); }} onError={() => setFailed(true)} /> : <div className="alya-image-fallback"><span>ALYA HOMES</span><small>{product.name}</small></div>}
