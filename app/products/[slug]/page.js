...
export default function ProductPage({ params }) {
  ...
  const inCart=cart.find(i=>i.slug===p.slug)?.qty||0;
  const remaining=stock===null?null:Math.max(0,stock-inCart);
  const outOfStock=remaining!==null&&remaining<=0;
  const addSelectedToCart=()=>{if(outOfStock)return; let success=false; for(let i=0;i<quantity;i++){if(addToCart(p))success=true;else break;} if(success){setAdded(true);setQuantity(q=>stock===null?q:Math.min(q,Math.max(1,stock-(inCart+quantity))));setTimeout(()=>setAdded(false),2200);}};
  const setSafeQuantity=next=>setQuantity(Math.max(1,remaining===null?next:Math.min(next,Math.max(1,remaining))));
  ...
  return <main className="alya-product-page">
    ...
    <section className="alya-product-main">
      ...
      <div className="alya-product-info">
        ...
        {stock!==null&&<div className={`alya-stock ${outOfStock?'is-empty':''}`}>{outOfStock?'Stokta yok':`${remaining} adet stokta`}</div>}
        <div className="alya-purchase-row">
          <div className="alya-quantity"><button type="button" onClick={()=>setSafeQuantity(quantity-1)} disabled={quantity<=1} aria-label="Adedi azalt"><Minus size={15}/></button><span>{quantity}</span><button type="button" onClick={()=>setSafeQuantity(quantity+1)} disabled={remaining!==null&&quantity>=remaining} aria-label="Adedi artır"><Plus size={15}/></button></div>
          {p.price==null?<button className="alya-primary-button" onClick={()=>setNotify(true)}><Mail size={18}/> Fiyat ve stok bilgisi al</button>:<button className="alya-primary-button" disabled={outOfStock} onClick={addSelectedToCart}>{added?<><Check size={19}/> Sepete eklendi</>:outOfStock?'Stokta yok':<><ShoppingBag size={19}/> Sepete ekle</>}</button>}
        </div>
        {added&&<div className="alya-cart-feedback"><Check size={16}/> {quantity} adet sepete eklendi. <Link href="/cart">Sepete git</Link></div>}
