...
export function StoreProvider({children}){
 ...
 const resolveProduct=(slug)=>{const base=customProducts.find(p=>p.slug===slug)||catalogProducts.find(p=>p.slug===slug); if(!base||deletedProducts.includes(slug))return null; const product={...base,...(productOverrides[slug]||{})}; if(product.active===false)return null; return product;};
 const getProduct=(slug)=>resolveProduct(slug);
 const getProducts=()=>[...catalogProducts,...customProducts].filter((p,i,a)=>a.findIndex(x=>x.slug===p.slug)===i&&!deletedProducts.includes(p.slug)).map(p=>({...p,...(productOverrides[p.slug]||{})})).filter(p=>p.active!==false);
 ...
 const resetProductOverride=async slug=>{setProductOverrides(o=>{const n={...o};delete n[slug];return n}); if(!serverReady)return; const base=catalogProducts.find(p=>p.slug===slug); if(!base)return; const payload={slug,name:base.name,code:base.code,category:base.category,price:base.price,description:base.description,image:base.image,active:true}; try{await fetch('/api/products',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)})}catch{}};
 ...
 const value=useMemo(()=>({cart,wishlist,customer,orders,productOverrides,customProducts,deletedProducts,serverReady,getProduct,getProducts,saveProductOverride,resetProductOverride,addProduct,deleteProduct,restoreProduct,updateOrderStatus,addToCart,validateCart,changeQty,remove,toggleWishlist,isWishlisted,saveCustomer,clearCustomer,createOrder,count:cart.reduce((s,i)=>s.qty,0),wishlistCount:wishlist.length,total:cart.reduce((s,i)=>s+(Number(i.price)||0)*i.qty,0)}),[cart,wishlist,customer,orders,productOverrides,customProducts,deletedProducts,serverReady]);
